...
if __name__ == '__main__':
    ...
    top_out_path = f"/home/shahar/projects/CVAE_proj/CVAE/outputs/{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    processes = []
    for l1o_idx, device in enumerate([DEVICE0,DEVICE0,DEVICE1,DEVICE1,DEVICE1]):#range(5):
        p = mp.Process(target=train_cvae, args=(l1o_idx, top_out_path, device, config))
        p.start()
        processes.append(p)
